...
function portal(node, target) {
  const $$d = user_derived(useStore), domNode = tag(user_derived(() => get($$d).domNode), "domNode");
  let destroyEffect;
  if (get(domNode)) {
...
function useInternalNode(id2) {
  const $$d = user_derived(useStore), nodeLookup = tag(user_derived(() => get($$d).nodeLookup), "nodeLookup"), nodes = tag(user_derived(() => get($$d).nodes), "nodes");
  const node = tag(
    user_derived(() => {
      get(nodes);
...
function MinimapNode($$anchor, $$props) {
  ...
  }), width = tag(user_derived(() => get($$d).width), "width"), height = tag(user_derived(() => get($$d).height), "height"), x = tag(user_derived(() => get($$d).x), "x"), y = tag(user_derived(() => get($$d).y), "y");
  var $$exports = { ...legacy_api() };
  var fragment = comment();
  var node = first_child(fragment);
  {
...
var root28 = add_locations(from_html(`<svelte-css-wrapper style="display: contents"><!></svelte-css-wrapper>`, 1), Minimap[FILENAME], [[74, 0]]);
...
var root_25 = add_locations(from_html(`<div><!></div>`), EdgeToolbar[FILENAME], [[34, 4]]);
...
function useUpdateNodeInternals() {
  const $$d = user_derived(useStore), domNode = tag(user_derived(() => get($$d).domNode), "domNode"), updateNodeInternals2 = tag(user_derived(() => get($$d).updateNodeInternals), "updateNodeInternals");
...
function useConnection() {
  const $$d = user_derived(useStore), connection = tag(user_derived(() => get($$d).connection), "connection");
  return {
    get current() {
...
function useNodeConnections({ id: id2, handleType, handleId, onConnect, onDisconnect } = {}) {
  const $$d = user_derived(useStore), edges = tag(user_derived(() => get($$d).edges), "edges"), connectionLookup = tag(user_derived(() => get($$d).connectionLookup), "connectionLookup");
  const contextNodeId = getNodeIdContext();
...
function useNodesData(nodeIds) {
  const $$d = user_derived(useStore), nodes = tag(user_derived(() => get($$d).nodes), "nodes"), nodeLookup = tag(user_derived(() => get($$d).nodeLookup), "nodeLookup");
  let prevNodesData = [];
  let initialRun = true;
  const nodeData = tag(
    user_derived(() => {
...
function useNodesInitialized() {
  const $$d = user_derived(useStore), nodesInitialized = tag(user_derived(() => get($$d).nodesInitialized), "nodesInitialized");
  return {
    get current() {
...
function useViewportInitialized() {
  const $$d_1 = user_derived(useStore), viewportInitialized = tag(user_derived(() => get($$d_1).viewportInitialized), "viewportInitialized");
  return {
    get current() {
